...
# Text Analysis & New Variable Creation

# There are many text column data, with ID and Name. We do not know how many
# unique values are there for each column. Using word cloud and word frequency, we get high
# level of understanding and this will also help in defining catergories if there are so many categories
# let's start with each text column
...
# Analysis on numerical data and relationship with categorical data
...
box_plot
box_plot = ggplot(train_data[train_data$revenue<100000000,], aes(y=revenue)) + geom_boxplot()
box_plot
...
# low budget movie and high budget movie, releasing at the same time?


#### Model Input Data Pre-paration ####
...
cor(val$logRevenue,h2o_rf_predict)
(sum(abs(val$logRevenue-h2o_rf_predict))/sum(val$logRevenue))*100 # validatio error
rm(h2o_rf_predict)

# test revenue prediction
h2o_rf_test_predict<-as.data.frame(h2o.predict(rfHex,testHex))
h2o_rf_test_predict = exp(h2o_rf_test_predict)
...
rmse(val$logRevenue, gbm_pred)
cor(gbm_pred,val$logRevenue)
...
rmse(val$logRevenue, xgb_pred)
cor(xgb_pred,val$logRevenue)
...
cor(val$logRevenue,h2o_aml_predict$predict)
(sum(abs(val$logRevenue-h2o_aml_predict$predict))/sum(val$logRevenue))*100 # validatio error
rm(h2o_aml_predict)
